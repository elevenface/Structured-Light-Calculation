...
// 为输入的矩阵申请空间
bool CDecode_Phase::AllocateSpace()
{
	// 判断参数是否合法
	if (this->m_numMat <= 0)
		return false;
...
// 删除所有空间
bool CDecode_Phase::DeleteSpace()
{
...
// 根据灰度图统计结果
bool CDecode_Phase::CountResult()
{
	...
	for (int i = 0; i < this->m_resRow; i++)
	{
		for (int j = 0; j < this->m_resLine; j++)
		{
			...
			// 计算偏移量
			float pix = (x) / (360)*(float)(this->m_pixPeroid);
			pix += 0.5;
...
// 解码
bool CDecode_Phase::Decode()
{
	// 根据灰度图恢复坐标
	if (!this->CountResult())
	{
		ErrorHandling("CDecode_Phase.Decode()->CountResult fault");
		return false;
	}
...
// 输入相应灰度图
bool CDecode_Phase::SetMat(int num, cv::Mat pic)
{
	if (this->m_grePicture == NULL)
	{
		ErrorHandling("CDecode_Phase.SetMat->grePicture Space is not allocated.");
		return false;
	}
...
// 设置参数
bool CDecode_Phase::SetNumMat(int numMat, int pixperiod)
{
	// 判断参数是否合法
	if ((numMat <= 0))
		return false;
